...
## 为什么基础固件（阶段 1 Release）里没有 nikki 和 fluent 主题？

这两个是第三方源码包（官方源里没有），已改为「构建插件包」工作流用官方 SDK 单独编译并发布到插件包 Release。这样插件更新只需约 15~30 分钟的 SDK 编译，不用 1.5~3 小时的固件全量重编。**日常使用的固件请用「快速定制构建」产出**——它自动带上最新插件；基础固件是纯净备用版。
