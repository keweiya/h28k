...
# 滚动快照（X.Y-SNAPSHOT / master）源码锁 version.buildinfo 的 revision——
# 正是官方当前产物编译的 commit，与正式版一样做 ABI 强校验（build_config.sh
# check-abi 三种形态统一校验）：配置合成一致（CLK_RK3528 已排除）则哈希相同；
# 官方轮换产物导致的失败重跑工作流即可。
